# Windows Package Script for Zerova OQC Report - Vietnam Factory Version
# This script builds the Vietnam factory version and packages it with all assets and config

$ErrorActionPreference = "Stop"

Write-Host "=========================================" -ForegroundColor Cyan
Write-Host "Zerova OQC Report - Vietnam Factory Package" -ForegroundColor Cyan
Write-Host "=========================================" -ForegroundColor Cyan
Write-Host ""
...
# Verify config.json has factory set to "vn"
$configPath = "assets\config.json"
if (Test-Path $configPath) {
    $configContent = Get-Content $configPath -Raw | ConvertFrom-Json
    if ($configContent.factory -ne "vn") {
        Write-Host "Warning: config.json factory is set to '$($configContent.factory)', expected 'vn'" -ForegroundColor Yellow
        Write-Host "Please ensure assets\config.json has 'factory': 'vn'" -ForegroundColor Yellow
    } else {
        Write-Host "✓ Config verified: factory = vn (Vietnam)" -ForegroundColor Green
    }
} else {
...
if (-not (Test-Path $exePath)) {
    Write-Host "`nBuild not found. Building Vietnam factory version..." -ForegroundColor Yellow
...
# Define paths with VN suffix
$packageDir = "dist\Zerova_OQC_Report_VN_$version"
$zipFile = "dist\Zerova_OQC_Report_v${version}_VN_Windows.zip"
...
} elseif (Test-Path "assets\config.json") {
    Copy-Item -Path "assets\config.json" -Destination "$packageDir\config.json" -Force
    Write-Host "  ✓ config.json copied from assets" -ForegroundColor Green
} else {
    Write-Host "  ⚠ config.json not found, using assets version only" -ForegroundColor Yellow
}
...
$readmeContent = @"
# Zerova OQC Report System v$version - Vietnam Factory
...
1. Extract all files from this package to a folder (e.g., C:\Program Files\Zerova_OQC_Report_VN)
...
The application uses `assets/config.json` from the data folder. The factory is set to "vn" (Vietnam).

If needed, you can also place a `config.json` file in the same directory as the executable.
...
Write-Host "Vietnam Factory Package Complete!" -ForegroundColor Green
Write-Host "=========================================" -ForegroundColor Cyan
