...
class _PackagePhotoManagerTabState extends State<PackagePhotoManagerTab> {
  ...
  Future<void> deleteSelectedImages() async {
    ...
    final existingList = prefs.getStringList('deleted_package_photos') ?? [];
    final updatedList = {...existingList, ...deletedFiles}.toList(); // 用 Set 去重複
    await prefs.setStringList('deleted_package_photos', updatedList);
    ...
    //bill12
    SharePointUploader(uploadOrDownload: 10, sn: '', model: widget.selectedModel).startAuthorization(
      categoryTranslations: {
        "packageing_photo": "Packageing Photo ",
      },
    );
